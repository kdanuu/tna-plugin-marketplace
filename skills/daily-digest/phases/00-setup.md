...
## 종료 조건
- `~/.claude/daily-digest.json`이 유효하고, 활성화된 소스/채널의 연동이 확인됨
- 슬랙 활성화 시 `$AGENTDECK_BOT_TOKEN` 환경변수 존재
...
2. 분기:
   ...
   - **JSON 파싱 실패 또는 `config_version` 필드 없음** →
     ...
     - Y → 온보딩 진행
     - N → 종료
   - **정상** → `sources`와 `channels`에서 `enabled: true`인 항목을 안내하고 Phase 1로:
     ```
     이전 설정을 확인했습니다.
   ...
   - **슬랙 활성화인데 `AGENTDECK_BOT_TOKEN` 환경변수 없음** →
     ...
     아래 "슬랙 선택 시" 1단계(토큰 입력 → settings.local.json 등록)만 실행.
     등록 확인 후 Phase 1로 진행.
...
**핵심**: 설치 → 인증 → 검증을 끊김 없이 한 흐름으로 실행한다.
...
설정은 이 단계에서만 원자적으로 저장한다 (중간 저장 없음).
...
```json
{
  ...
  "sources": {
    "caret": { "enabled": true/false },
    "gemini": { "enabled": true/false }
  },
  "channels": {
    "slack": {
      "enabled": true/false,
...
온보딩 완료 후 자동으로 Phase 1로 진행한다.

### 온보딩 실패 처리

- 소스 0개 → "최소 1개의 회의록 소스가 필요합니다" → 소스 선택으로 복귀
- 채널 0개 → "최소 1개의 전송 채널이 필요합니다" → 채널 설정으로 복귀
